...
param blobStorageTypesName string = 'blob${uniqueString(resourceGroup().id)}'
param fileStorageTypesName string = 'file${uniqueString(resourceGroup().id)}'
param tableStorageTypesName string = 'table${uniqueString(resourceGroup().id)}'
...
@description('The name of the management public IP address')
param managementIPadressName string = 'IpAddress-${uniqueString(resourceGroup().id)}'
@description('The name of the webserver public IP address')
param webserverIPadressName string = 'IpAddress-${uniqueString(resourceGroup().id)}'
...
resource table 'tableServices@2023-01-01' = {
  name: tableStorageTypesName
}
}
    ...
    ipConfigurations:[
      {
        ...
        properties:{
          ...
          privateIPAddress:managementIP.id
          subnet:{
            name:managementSubnetName
            properties:{
              addressPrefix:managementVnetPrefix
            }
            id: '${managementvnet.id}/subnets/webserversubnet'
          }
        }
      }
    ]
